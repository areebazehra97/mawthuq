  ...
  const approved = useMemo(() => {
    return vendors
      .map((vendor) => {
        ...
        const scorecard = buildVendorScorecard(vendor, vendorDocuments, config);
        const gradeField = seededAiExtractions[vendor.id]?.find(
          ...
          <table className="min-w-full text-left">
            ...
            <tbody>
              {approved.map(({ vendor, scorecard, grade }) => (
                <tr key={vendor.id} className="border-b border-slate-100 last:border-b-0 align-top">
                  ...
                  <td className="py-5 pr-4 text-sm font-semibold text-slate-900">{grade}</td>
                  <td className="py-5 pr-4 text-sm text-slate-600">{vendor.primaryDiscipline}</td>
                  <td className="py-5 pr-4">
                    <RiskBadge riskLevel={scorecard.riskLevel} />
                  </td>
                  <td className="py-5 pr-4 text-sm text-slate-600">{vendor.expiryRisk}</td>
...
function ActionModal({
  ...
  const scorecard = buildVendorScorecard(modalState.vendor, vendorDocuments, config);
  const extractions = seededAiExtractions[modalState.vendor.id] ?? [];
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/55 p-4">
      <div className="mx-auto mt-8 max-w-5xl rounded-[32px] border border-white/10 bg-white p-6 shadow-[0_30px_100px_rgba(0,0,0,0.35)]">
        ...
        {modalState.kind === "report" ? (
          <div className="mt-6 space-y-6">
            <div className="grid gap-4 md:grid-cols-3">
              <PreviewMetric label="Vendor" value={modalState.vendor.name} />
              <PreviewMetric label="Decision" value={scorecard.decision} />
              <PreviewMetric label="Score" value={String(scorecard.overallScore)} />
            </div>
              ...
              <CardContent className="space-y-3">
                {scorecard.findings.map((finding) => (
                  <div key={finding.id} className="rounded-2xl border border-slate-200 p-4">
                    ...
                    <p className="mt-2 text-sm text-slate-500">{finding.explanation}</p>
                  </div>
                ))}
              </CardContent>
            </Card>
